...
//						"AND UserID = " + userID
		...
		/// <summary>
		/// Validates a username and password combination and, if there is a match, returns a user detail object including user data with token, variable lifetime (max 20 minutes) and resource identification for 2 factor authentication.
		/// </summary>
		...
		public UserDetail UserLogin2FA(string username, string password, int expirationMinutes)
        {
            ...
            using (var userReader = executeReader(@"SELECT u.UserID, u.LID, u.Enable2FA, u.SponsorID 
                                                    FROM [User] u WHERE u.Username = @Username AND u.Password = @Password", new SqlParameter("@Username", username), new SqlParameter("@Password", HashMD5(password.Trim()))))
            {
                if (userReader.Read())
                {
                    ...
                    using (var rs = executeReader(@"SELECT Enable2FA FROM Sponsor 
                    ...
                    if (enable2FA)
                    {
                        ...
                        using (var loginReader = executeReader(@"SELECT ul.ResourceID
                                                                FROM UserLogin ul
                                                                WHERE ul.UserID = @UserID
                                                                AND DATEDIFF(MINUTE, ul.LoginAttempt, GETDATE()) < @Minute
                                                                AND ISNULL(ul.Unblocked, 0) = 0", new SqlParameter("@UserID", userID), new SqlParameter("@Minute", MINUTE)))
                        {
                            ...
                            else
                            {
                                ...
                                using (var checkIfSecretkeyExist = executeReader(@"SELECT us.SecretKey 
                                                                       FROM UserSecret us
                                                                       WHERE us.UserID = @UserID", new SqlParameter("@UserID", userID)))
                                {
                                    if (!checkIfSecretkeyExist.Read())
                                    {
                                        if (checkSecretKeyExist(checkIfSecretkeyExist.FieldCount))
                                        {
                                            ...
                                            executeNonQuery(
                                                ...
                                                VALUES(@UserID, @IPAddress, GETDATE(), @ResourceID, @UserToken, @FromWebService)",
                                                new SqlParameter("@UserID", userID),
...
AND ISNULL(FromWebsite, 0) = 1", new SqlParameter("@UserID", userID), new SqlParameter("@Minute", MINUTE))) {
		...
		private string getString(SqlDataReader rs, int index, string def)
		{
			return rs.IsDBNull(index) ? def : rs.GetString(index);
		}
		private int getInt32(SqlDataReader rs, int index)
		{
			return getInt32(rs, index, 0);
		}
		private int getInt32(SqlDataReader rs, int index, int def)
		{
			return rs.IsDBNull(index) ? def : rs.GetInt32(index);
		}
		private SqlDataReader executeReader(string sqlString, params SqlParameter[] parameters)
		{
			return executeReader(sqlString, "SqlConnection", parameters);
		}
		private SqlDataReader executeReader(string sqlString, string con, params SqlParameter[] parameters)
		{
		...
		private int executeNonQuery(string sqlString, params SqlParameter[] parameters)
		{
			return executeNonQuery(sqlString, "SqlConnection", parameters);
		}
		private int executeNonQuery(string sqlString, string con, params SqlParameter[] parameters)
		{
			SqlConnection dataConnection = new SqlConnection(ConfigurationManager.ConnectionStrings[con].ConnectionString);
			dataConnection.Open();
			SqlCommand dataCommand = new SqlCommand(sqlString, dataConnection);
