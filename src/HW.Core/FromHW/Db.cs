...
namespace HW.Core.FromHW
{
	public class Db
	{
		public static string rightNowHealth(int sponsorID, int LID)
		{
			System.Text.StringBuilder ret = new System.Text.StringBuilder();

			string rating = "", color = "";
			int val = getHealth(sponsorID);
			//getRating(val, ref rating, ref color);
			rating = "high";
			color = "green";
			ret.Append("<div class=\"landing_graph\">");
			ret.Append("<img src=\"/piechart.aspx?C=" + color + "&V=" + val + "&" + DateTime.Today.ToString("yyyyMMdd") + "\" width=\"80\" height=\"80\" alt=\"Health " + val + "%\" />");
			ret.Append("<br />");
		...
		public static string rightNowSleep(int sponsorID, int LID)
		{
			System.Text.StringBuilder ret = new System.Text.StringBuilder();

			string rating = "", color = "";
			int val = getSleep(sponsorID);
			//getRating(val, ref rating, ref color);
			rating = "med";
			color = "orange";
			ret.Append("<div class=\"landing_graph\">");
			ret.Append("<img src=\"/piechart.aspx?C=" + color + "&V=" + val + "&" + DateTime.Today.ToString("yyyyMMdd") + "\" width=\"80\" height=\"80\" alt=\"Sleep " + val + "%\" />");
			ret.Append("<br />");
			ret.Append("<img src=\"/images/" + color + "Shadow.png\" width=\"80\" height=\"22\" />");
			ret.Append("</div>");
			ret.Append("<div class=\"metadata\">");
			...
			switch (LID)
			{
					case 1: ret.Append("<div class=\"label\">av våra användare <br /> sov mycket eller ganska <br />bra igår natt.</div>"); break;
					default: ret.Append("<div class=\"label\">of our users <br /> slept very or quite <br />good yesterday.</div>"); break;
			}
			ret.Append("</div>");

			return ret.ToString();
		}
		public static string rightNowWorkAtmosphere(int sponsorID, int LID)
		{
			System.Text.StringBuilder ret = new System.Text.StringBuilder();

			string rating = "", color = "";
			int val = getWorkAtmosphere(sponsorID);
			//getRating(val, ref rating, ref color);
			rating = "low";
			color = "purple";
			ret.Append("<div class=\"landing_graph\">");
			ret.Append("<img src=\"/piechart.aspx?C=" + color + "&V=" + val + "&" + DateTime.Today.ToString("yyyyMMdd") + "\" width=\"80\" height=\"80\" alt=\"Work atmosphere " + val + "%\" />");
			ret.Append("<br />");
			ret.Append("<img src=\"/images/" + color + "Shadow.png\" width=\"80\" height=\"22\" />");
			ret.Append("</div>");
			ret.Append("<div class=\"metadata\">");
			...
			switch (LID)
			{
					case 1: ret.Append("<div class=\"label\">av våra användare hade mycket eller ganska bra stämning på jobbet igår.</div>"); break;
					default: ret.Append("<div class=\"label\">of our users had very or quite good work atmosphere yesterday.</div>"); break;
			}
			ret.Append("</div>");
		...
		public static string rightNow(int sponsorID, int LID)
		{
			...
			ret.Append(rightNowHealth(sponsorID, LID));
			ret.Append(rightNowSleep(sponsorID, LID));
			ret.Append(rightNowWorkAtmosphere(sponsorID, LID));
			
			ret.Append("</div>");

			return ret.ToString();
		}
		private static int getUsers(int sponsorID)
		{
			if (HttpContext.Current.Application["UsersToday" + (sponsorID != 0 ? sponsorID.ToString() : "")] == null)
			{
				SqlDataReader rs = Db.rs("SELECT COUNT(*) " +
				                         ...
				                         "WHERE av.QuestionID = 239 AND av.OptionID = 288 AND a.EndDT >= '" + DateTime.Today.AddDays(-1) + "'", "eFormSqlConnection");
				if (rs.Read())
				{
					HttpContext.Current.Application["UsersToday" + (sponsorID != 0 ? sponsorID.ToString() : "")] = rs.GetInt32(0);
				}
				rs.Close();
			}
			return Convert.ToInt32(HttpContext.Current.Application["UsersToday" + (sponsorID != 0 ? sponsorID.ToString() : "")]);
		}
		public static int getWorkAtmosphere(int sponsorID)
		{
			if (getUsers(sponsorID) > 0 && HttpContext.Current.Application["WorkAtmosphereToday" + (sponsorID != 0 ? sponsorID.ToString() : "")] == null)
			{
				SqlDataReader rs = Db.rs("SELECT COUNT(*) " +
				                         "FROM Answer a " +
				                         "INNER JOIN AnswerValue av ON a.AnswerID = av.AnswerID " +
				                         (sponsorID != 0 ? "INNER JOIN healthwatch..SponsorProjectRoundUnit spru ON a.ProjectRoundUnitID = spru.ProjectRoundUnitID AND spru.SponsorID = " + sponsorID + " " : "") +
				                         "WHERE av.QuestionID = 248 AND av.OptionID = 54 AND av.ValueInt > 62 AND a.EndDT >= '" + DateTime.Today.AddDays(-1) + "'", "eFormSqlConnection");
				if (rs.Read())
				{
					HttpContext.Current.Application["WorkAtmosphereToday" + (sponsorID != 0 ? sponsorID.ToString() : "")] = Convert.ToInt32((double)rs.GetInt32(0) / (double)getUsers(sponsorID) * 100);
				}
				rs.Close();
			}
			return (HttpContext.Current.Application["WorkAtmosphereToday" + (sponsorID != 0 ? sponsorID.ToString() : "")] == null ? 0 : Convert.ToInt32(HttpContext.Current.Application["WorkAtmosphereToday" + (sponsorID != 0 ? sponsorID.ToString() : "")]));
		}
		public static int getSleep(int sponsorID)
		{
			if (getUsers(sponsorID) > 0 && HttpContext.Current.Application["SleepToday" + (sponsorID != 0 ? sponsorID.ToString() : "")] == null)
			{
				SqlDataReader rs = Db.rs("SELECT COUNT(*) " +
				                         "FROM Answer a " +
				                         "INNER JOIN AnswerValue av ON a.AnswerID = av.AnswerID " +
				                         (sponsorID != 0 ? "INNER JOIN healthwatch..SponsorProjectRoundUnit spru ON a.ProjectRoundUnitID = spru.ProjectRoundUnitID AND spru.SponsorID = " + sponsorID + " " : "") +
				                         "WHERE av.QuestionID = 239 AND av.OptionID = 288 AND av.ValueInt > 62 AND a.EndDT >= '" + DateTime.Today.AddDays(-1) + "'", "eFormSqlConnection");
				if (rs.Read())
				{
					HttpContext.Current.Application["SleepToday" + (sponsorID != 0 ? sponsorID.ToString() : "")] = Convert.ToInt32((double)rs.GetInt32(0) / (double)getUsers(sponsorID) * 100);
				}
				rs.Close();
			}
			return (HttpContext.Current.Application["SleepToday" + (sponsorID != 0 ? sponsorID.ToString() : "")] == null ? 0 : Convert.ToInt32(HttpContext.Current.Application["SleepToday" + (sponsorID != 0 ? sponsorID.ToString() : "")]));
		}
		public static int getHealth(int sponsorID)
		{
			if (getUsers(sponsorID) > 0 && HttpContext.Current.Application["HealthToday" + (sponsorID != 0 ? sponsorID.ToString() : "")] == null)
			{
				SqlDataReader rs = Db.rs("SELECT COUNT(*) " +
				                         "FROM Answer a " +
				                         "INNER JOIN AnswerValue av ON a.AnswerID = av.AnswerID " +
				                         (sponsorID != 0 ? "INNER JOIN healthwatch..SponsorProjectRoundUnit spru ON a.ProjectRoundUnitID = spru.ProjectRoundUnitID AND spru.SponsorID = " + sponsorID + " " : "") +
				                         "WHERE av.QuestionID = 238 AND av.OptionID = 55 AND av.ValueInt > 62 AND a.EndDT >= '" + DateTime.Today.AddDays(-1) + "'", "eFormSqlConnection");
				if (rs.Read())
				{
					HttpContext.Current.Application["HealthToday" + (sponsorID != 0 ? sponsorID.ToString() : "")] = Convert.ToInt32((double)rs.GetInt32(0) / (double)getUsers(sponsorID) * 100);
				}
				rs.Close();
			}
			return (HttpContext.Current.Application["HealthToday" + (sponsorID != 0 ? sponsorID.ToString() : "")] == null ? 0 : Convert.ToInt32(HttpContext.Current.Application["HealthToday" + (sponsorID != 0 ? sponsorID.ToString() : "")]));
		}
						...
						if (rs2.IsDBNull(2))
						{
							...
							for (int i = 0; i < units.Length; i++)
							{
								...
								else
								{
									...
									{
										parent = rs3.GetInt32(0);
										Db.exec("UPDATE ProjectRoundUnit SET ID = dbo.cf_unitExtID(ProjectRoundUnitID,dbo.cf_unitDepth(ProjectRoundUnitID),''), SortOrder = ProjectRoundUnitID WHERE ProjectRoundUnitID = " + parent, "eFormSqlConnection");
