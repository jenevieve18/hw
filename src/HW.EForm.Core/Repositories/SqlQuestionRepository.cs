...
VALUES(
	...
	@Niner
			...
			ExecuteNonQuery(
				...
				new SqlParameter("@LimitWidth", question.LimitWidth),
				new SqlParameter("@FillRemainderWithBgColor", question.FillRemainderWithBgColor),
				new SqlParameter("@Niner", question.Niner)
			);
		}
...
UPDATE Question SET
	...
	FillRemainderWithBgColor = @FillRemainderWithBgColor,
	Niner = @Niner
			...
			using (var rs = ExecuteReader(query, new SqlParameter("@QuestionID", id))) {
				if (rs.Read()) {
					question = new Question {
						...
						Box = GetInt32(rs, 10),
						LimitWidth = GetInt32(rs, 11),
						FillRemainderWithBgColor = GetString(rs, 12),
						Niner = GetInt32(rs, 13)
					};
				}
			}
			return question;
