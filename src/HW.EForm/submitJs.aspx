<%@ Page language="c#" Codebehind="submitJs.aspx.cs" AutoEventWireup="false" Inherits="eForm.submitJs" ContentType="application/x-javascript; charset=utf-8" %>
window.history.forward(1);

function sendEmail(ak)
{
	var email = prompt('Email','');
	location.href='sendPDF.aspx?AK='+ak+'&Email='+email;
}
...
function isValBetween(r,l,h)
{ 
	return (r.value >= l && r.value <= h); 
}
...
function updateButtons(act,f)
{
	...
	if(f)
	{
		...
		if(reqd > cmpl)
		{
			...
			{
				s.src = 'submitImages/button_send<%=HttpContext.Current.Request.QueryString["LID"]%>_0.gif';
			}
			else if(s = document.getElementById('NextCtrl'))
			{
				s.src = 'submitImages/button_next<%=HttpContext.Current.Request.QueryString["LID"]%>_0.gif';
			}
		}
		else if(cmpl >= reqd)
		{
			if(s = document.getElementById('SendCtrl'))
			{
				s.src = 'submitImages/button_send<%=HttpContext.Current.Request.QueryString["LID"]%>_1.gif';
			}
			else if(s = document.getElementById('NextCtrl'))
			{
				s.src = 'submitImages/button_next<%=HttpContext.Current.Request.QueryString["LID"]%>_1.gif';
			}
		}
		if(a != 0)
		{
	...
	if(document.getElementById('progressBar').src != n)
	{
		...
		else if(m > 55)
		{
			...
			if(m > 1)
			{
				document.getElementById('minutes').innerHTML = '<%=(HttpContext.Current.Request.QueryString["LID"].ToString() == "2" ? "About" : "Ca.") %> ' + m + ' <%=(HttpContext.Current.Request.QueryString["LID"].ToString() == "2" ? "minutes left" : "minuter kvar") %>';
			}
			else
			{
				document.getElementById('minutes').innerHTML = '<%=(HttpContext.Current.Request.QueryString["LID"].ToString() == "2" ? "About 1 minute left" : "Ca. 1 minut kvar") %>';
			}
		}
		else
		{
			document.getElementById('minutes').innerHTML = '<%=(HttpContext.Current.Request.QueryString["LID"].ToString() == "2" ? "About" : "Ca.") %> ' + m + ' <%=(HttpContext.Current.Request.QueryString["LID"].ToString() == "2" ? "seconds left" : "sekunder kvar") %>';
		}
	}
}
...
function updatePage()
{
	...
	if(cp<=pCX)
	{
		if(cp==pCX && cp > 1)
		{
			document.getElementById('status').innerHTML = '<%=(HttpContext.Current.Request.QueryString["LID"].ToString() != "1" ? "Last page!" : "Sista sidan!") %>';
		}
		else
		{
			document.getElementById('status').innerHTML = '<%=(HttpContext.Current.Request.QueryString["LID"].ToString() != "1" ? "Page" : "Sidan") %> ' + cp + ' <%=(HttpContext.Current.Request.QueryString["LID"].ToString() != "1" ? "of" : "av") %> ' + pCX;
		}
	}
	else
	{
...
function showProcessing()
{
	try
	{
		...
		{
			currentMiddle = document.documentElement.scrollTop + document.documentElement.clientHeight/2 - 200;
		}
		else
		{
			currentMiddle = document.body.scrollTop + document.body.clientHeight/2 - 200;
		}
		
		pW = document.getElementById('processingWindow');
		pW.style.top = currentMiddle;
		pW.style.display = '';
	}
	catch(Exception) {}
}