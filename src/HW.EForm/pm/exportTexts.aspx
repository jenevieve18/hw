...
<%
	...
	System.Data.SqlClient.SqlDataReader rs = eform.Db.sqlRecordSet("SELECT " +
		...
		"av.ValueText, " +
		"av.QuestionID, " +
		"(SELECT ISNULL(ocl.Text,CAST(av2.ValueInt AS VARCHAR(5))) FROM AnswerValue av2 LEFT OUTER JOIN OptionComponent oc ON av2.ValueInt = oc.OptionComponentID AND av2.ValueInt > 100 LEFT OUTER JOIN OptionComponentLang ocl ON oc.OptionComponentID = ocl.OptionComponentID AND ocl.LangID = q.LangID WHERE av2.DeletedSessionID IS NULL AND av2.AnswerID = av.AnswerID AND av2.QuestionID = av.QuestionID-1), " +
		"(SELECT ql2.Question FROM QuestionLang ql2 WHERE ql2.LangID = q.LangID AND ql2.QuestionID = q.QuestionID-1), " +
		"a.AnswerID, " +
		"av.OptionID, " +
		"qq.Variablename, " +
	...
	while(rs.Read())
	{
		...
		{
%>
</table><br><table border=1 cellpadding=2 cellspacing=2><tr><td>AnswerID</td><!--<td><%=rs.GetString(4)%></td>--><td colspan="2">[<% if(rs.IsDBNull(7)||rs.GetString(7).Trim()==""){%>Q<%=rs.GetInt32(2)%>O<%=rs.GetInt32(6)%><%}else{%><%=rs.GetString(7)%><%}%>] <b><%=(rs.IsDBNull(0) ? rs.GetInt32(2).ToString() : rs.GetString(0))%></b></td></tr>
<%
		}
		SQ = rs.GetInt32(2);
%>
<tr><td><%=rs.GetInt32(5)%></td><!--<td><%=(rs.IsDBNull(3) ? "-" : rs.GetString(3).ToString().Replace("\r\n","<BR>"))%></td>--><td><%=rs.GetString(1).Replace("\r\n","<BR>")%></td><td><%=(rs.IsDBNull(8) ? "" : rs.GetString(8).Replace("\r\n","<BR>"))%></td></tr>
<%
