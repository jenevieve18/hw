...
{
	public class SqlProjectRoundUnitManagerRepository : BaseSqlRepository<ProjectRoundUnitManager>, IProjectRoundUnitManagerRepository
	{
...
FROM ProjectRoundUnitManager";
		...
		public IList<ProjectRoundUnitManager> FindByProjectRoundUnit(int projectRoundUnitID)
		{
			string query = @"
SELECT 	ProjectRoundUnitManagerID, 
	ProjectRoundUnitID, 
...
WHERE ProjectRoundUnitID = @ProjectRoundUnitID";
			var projectRoundUnitManagers = new List<ProjectRoundUnitManager>();
			using (var rs = ExecuteReader(query, new SqlParameter("@ProjectRoundUnitID", projectRoundUnitID))) {
				while (rs.Read()) {
					projectRoundUnitManagers.Add(new ProjectRoundUnitManager {
						ProjectRoundUnitManagerID = GetInt32(rs, 0),
						ProjectRoundUnitID = GetInt32(rs, 1),
